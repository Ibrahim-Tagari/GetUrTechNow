"use client"

import { SiteHeader } from "@/components/site-header"
import { SiteFooter } from "@/components/site-footer"
import { ProductGrid } from "@/components/product-grid"
import { ProductFilters } from "@/components/product-filters"
import { useState, useMemo, useEffect } from "react"
import type { FilterState } from "@/components/product-filters"

const defaultSmartphones = [
  {
    id: "1",
    name: "iPhone 15 Pro Max 256GB",
    price: 24999,
    originalPrice: 26999,
    image: "/iphone-15-pro-max.jpg",
    rating: 4.9,
    reviews: 542,
    inStock: true,
    badge: "Best Seller",
    brand: "Apple",
    processor: "Apple A17",
  },
  {
    id: "2",
    name: "Samsung Galaxy S24 Ultra",
    price: 22999,
    image: "/samsung-galaxy-s24-ultra.jpg",
    rating: 4.8,
    reviews: 487,
    inStock: true,
    brand: "Samsung",
    processor: "Snapdragon 8 Gen 3",
  },
  {
    id: "3",
    name: "Google Pixel 8 Pro",
    price: 18999,
    originalPrice: 20999,
    image: "/google-pixel-8-pro.jpg",
    rating: 4.7,
    reviews: 324,
    inStock: true,
    badge: "Sale",
    brand: "Google",
    processor: "Google Tensor G3",
  },
  {
    id: "4",
    name: "OnePlus 12 5G",
    price: 15999,
    image: "/oneplus-12-smartphone.jpg",
    rating: 4.6,
    reviews: 256,
    inStock: true,
    brand: "OnePlus",
    processor: "Snapdragon 8 Gen 3",
  },
  {
    id: "5",
    name: "Xiaomi 14 Pro",
    price: 16999,
    image: "/xiaomi-14-pro.jpg",
    rating: 4.7,
    reviews: 298,
    inStock: true,
    brand: "Xiaomi",
    processor: "Snapdragon 8 Gen 3",
  },
  {
    id: "6",
    name: "iPhone 14 128GB",
    price: 17999,
    originalPrice: 19999,
    image: "/iphone-14.jpg",
    rating: 4.8,
    reviews: 612,
    inStock: true,
    badge: "Sale",
    brand: "Apple",
    processor: "Apple A16",
  },
  {
    id: "7",
    name: "Samsung Galaxy A54 5G",
    price: 8999,
    image: "/samsung-galaxy-a54.jpg",
    rating: 4.5,
    reviews: 423,
    inStock: true,
    badge: "Budget Pick",
    brand: "Samsung",
    processor: "Exynos 1380",
  },
  {
    id: "8",
    name: "OPPO Find X6 Pro",
    price: 19999,
    image: "/oppo-find-x6-pro.jpg",
    rating: 4.6,
    reviews: 187,
    inStock: false,
    brand: "OPPO",
    processor: "Snapdragon 8 Gen 2",
  },
  {
    id: "9",
    name: "Huawei P60 Pro",
    price: 17999,
    image: "/huawei-p60-pro.jpg",
    rating: 4.5,
    reviews: 156,
    inStock: true,
    brand: "Huawei",
    processor: "Snapdragon 8+ Gen 1",
  },
]

export default function SmartphonesPage() {
  const [smartphones, setSmartphones] = useState(defaultSmartphones)
  const [filters, setFilters] = useState<FilterState>({
    priceRange: [0, 60000],
    brands: [],
    processors: [],
    inStockOnly: false,
  })

  useEffect(() => {
    const adminProducts = JSON.parse(localStorage.getItem("admin_products") || "[]")
    const smartphoneProducts = adminProducts.filter((p: any) => p.category === "smartphones")
    setSmartphones([...defaultSmartphones, ...smartphoneProducts])
  }, [])

  const filteredProducts = useMemo(() => {
    return smartphones.filter((product) => {
      if (product.price < filters.priceRange[0] || product.price > filters.priceRange[1]) {
        return false
      }

      if (filters.brands.length > 0 && !filters.brands.includes(product.brand)) {
        return false
      }

      if (filters.processors.length > 0 && !filters.processors.includes(product.processor)) {
        return false
      }

      if (filters.inStockOnly && !product.inStock) {
        return false
      }

      return true
    })
  }, [filters, smartphones])

  return (
    <div className="flex min-h-screen flex-col">
      <SiteHeader />
      <main className="flex-1">
        {/* Page Header */}
        <div className="bg-muted/30 border-b">
          <div className="container mx-auto px-4 py-8">
            <h1 className="text-4xl font-bold mb-2">Smartphones</h1>
            <p className="text-muted-foreground text-lg">Latest mobile devices with cutting-edge technology</p>
          </div>
        </div>

        {/* Products Section */}
        <div className="container mx-auto px-4 py-8">
          <div className="flex flex-col lg:flex-row gap-8">
            {/* Filters Sidebar */}
            <aside className="lg:w-64 flex-shrink-0">
              <ProductFilters category="smartphones" onFilterChange={setFilters} />
            </aside>

            {/* Products Grid */}
            <div className="flex-1">
              <div className="flex items-center justify-between mb-6">
                <p className="text-sm text-muted-foreground">{filteredProducts.length} products found</p>
              </div>
              <ProductGrid products={filteredProducts} />
            </div>
          </div>
        </div>
      </main>
      <SiteFooter />
    </div>
  )
}
